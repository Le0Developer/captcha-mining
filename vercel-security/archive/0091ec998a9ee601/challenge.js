...
class C_6_1_ {
  ...
  start() {
    this.failTimeout = setTimeout(() => f_1_2_2("99"), 15000);
    new v_1_4("/.well-known/vercel/security/static/challenge.v2.min.js").postMessage({
      port: this.chan.port1
    }, [this.chan.port1]);
    this.postMessage({
      type: "solve-request",
      token: vLStoken_4_,
  ...
  postMessage(p_1_6) {
    this.chan.port2.postMessage(p_1_6);
  }
  async processEvalRequest(p_1_7) {
    const {
      id: t,
      argv: d
    } = p_1_7;
    try {
      const v_1_5 = await v_1_3(d);
      this.postMessage({
        type: "eval-response",
        id: t,
        value: v_1_5,
  ...
  async onMessage(p_7_) {
    if (p_7_.data.token === vLStoken_4_) {
      switch (p_7_.data.type) {
        ...
        case "eval-request":
          {
            await this.processEvalRequest(p_7_.data);
            break;
          }
      }
    }
