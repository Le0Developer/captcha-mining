...
async function f_3_1_(p_1_2, p_1_3, p_1_4) {
  const v_3_2 = await fetch("/.well-known/vercel/security/request-challenge", {
    method: "POST",
    headers: {
  ...
  if (!v_3_2.ok) {
    throw new Error(v_3_2.statusText);
  }
  return v_3_2;
}
...
async function f_1_1_2(p_3_) {
  await f_0_1_2();
  try {
    const v_1_3 = await Solve(p_3_.token);
    await f_3_1_(p_3_.token, v_1_3, p_3_.version);
    f_1_3_({
      type: "solve-response",
      success: true,
      token: vO_2_8_.token
...
function f_1_1_3(p_8_) {
  if (f_1_1_4(p_8_)) {
    switch (p_8_.data.type) {
      ...
      case "eval-response":
        {
          const v_2_4 = v_3_.get(p_8_.data.id);
          if ("value" in p_8_.data) {
            v_2_4?.resolve(p_8_.data.value);
          } else {
            v_2_4?.reject(p_8_.data.error);
          }
